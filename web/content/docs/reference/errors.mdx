---
title: Error strings
description: Messages the CLI prints, which code path emits them, and what to do next when SSH, staging, or an empty batch fails.
icon: Ban
---
...
| `Use either --all or --id, not both` | `vmup prune --all --id` |
...
| `Upload failed: <err>` + `Local staging kept: <dir>` | ssh put or remote finalize |
...
| `(copied prompt to clipboard)` | Human upload, `clipboard_copy = "prompt"` (default) |
| `(copied agent folder path to clipboard)` | Human upload, `clipboard_copy = "path"` |
| `(clipboard copy unavailable)` | `pbcopy` / `wl-copy` / `xclip` failed. Upload still succeeded. |
...
| `Pruned N remote batch(es)` | Age prune or `--all` |
