...
vmup shells out to your `ssh`. It does not implement SSH itself. `vmup check` is the retry path that skips the init wizard.
...
Preflight timeout is **20 seconds**. Upload timeout is **10 minutes** per file.
...
SSH works; `~/vmup/.cleanup.sh` is not executable on the host. Batches still upload. They may outlive TTL if this machine sleeps. Reinstall the sweeper, or delete them now with `vmup prune --all`.
