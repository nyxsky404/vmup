---
title: config.toml
description: config.toml path, keys, defaults, and profiles for hosts, TTL, file limits, and SSH settings.
icon: FileCog
---
...
| `clipboard_copy` | string | `"prompt"` | No. `prompt`, `path`, or `none`. Invalid values fall back to `prompt`. Not asked at init. |
...
After a successful human-mode upload, vmup copies `clipboard_copy` to the local clipboard: the rendered **prompt** (default), the folder **path**, or **none**. `--json` never copies.
...
`emptyTemplateConfig()` seeds a `default` profile with user `ubuntu`, key hint `~/.ssh/id_ed25519`, port 22, remote `~/vmup`, TTL 5, `accept_all_files true`, `max_file_mb 200`, `clip_dedup true`, `clipboard_copy "prompt"`. Init then overwrites fields from the wizard. `clipboard_copy` is written for new files and is not asked.
...
After you change `ttl_minutes`, run `vmup check --sweeper` so the remote script’s `-mmin` matches. Editing the file alone does not rewrite `.cleanup.sh`. `vmup prune --all` deletes every remote batch now and does not read this cutoff.
