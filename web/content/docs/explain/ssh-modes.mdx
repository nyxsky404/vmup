...
description: Choose direct SSH (host, user, key, port) or an OpenSSH alias (ssh_host) from ~/.ssh/config. vmup still calls ssh.
icon: Cable
---
...
File uploads reuse that ssh: the local file is piped to `cat > <remote-file>` so the spinner can count bytes as they are written.
...
A `Host` block that already sets `BatchMode` or `accept-new` is fine.

## Remote paths and `~`

vmup expands `~` via `$HOME` in remote shell snippets (`mkdir`, `cat`, `mv`, `rm`, the sweeper). Displayed agent paths keep the `~/vmup/…` form.
