---
title: First upload
description: Walk through install, vmup init, an SSH check, one file upload, and pasting the remote folder path into the agent.
icon: Rocket
---
...
```text
⠋ Checking SSH…
⠋ Uploading 1 file  240.0 KB / 240.0 KB  2s
```
...
stderr adds `(copied agent folder path to clipboard)` when `pbcopy` (macOS), `wl-copy`, or `xclip` works.
